...
class SupportMessageUseCase:
    ...
    async def create_support_message(
        ...
        try:
            ...
            logger.error('error', f'failed creating support message, detail: {str(e)}')
            return {'status': 'failed creating support message', 'detail': str(e)}


def get_support_message_use_case(
    session: AsyncSession = Depends(db_config.session),
    support_message_repository: SupportMessageRepository = Depends(get_support_message_repository)
